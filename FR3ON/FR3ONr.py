...
async def for_users (bot,m):
   if check(m.from_user.id):
     ...
     return await m.reply_text(f"☭ اهلا بيك عزيزي المطور  ⁽ {m.from_user.mention} ₎\n ☭ اليك مصانع تليثون سورس هورس\n ☭ قناة  السورس ⇇ @H_0RSE", reply_markup=Keyboard)
   kep = ReplyKeyboardMarkup([["《صنع بوت》", "《حذف بوت》"], ["《السورس》", "《مطور السورس》"]], resize_keyboard=True)
   await m.reply_text(f"☭ اهلا بيك عزيزي  ⁽ {m.from_user.mention} ₎\n☭ اليك مصانع تليثون سورس \nقناة السورس ⇇ @H_0RSE", reply_markup=kep)
  ...
  if m.text in admins_commands:
    if not check(m.from_user.id):
      ...
      if m.text == 'ضع قناة الاشتراك':
        await m.reply("☭ ارسل معرف القناة العام مثال @H_0RSE", quote=True)
        ...
        if len(get_admins()) == 0:
          ...
          for admin in get_admins():
            try:
              ...
              text += f'☭ [@H_0RSE](tg://user?id={admin})\n'
          await m.reply(text, quote=True)
